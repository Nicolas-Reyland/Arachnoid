...
def hashify_dir(dir_path, calc_hash=False):
	...
	for obj in listdir(dir_path):
		...
		if isfile(obj):
			...
			yield (('d', obj), dict(hashify_dir(obj, calc_hash)))
		else:
			print(f'WARNING: Ignoring object {obj} which is neither a file, not a directory')

def save_obj(dir_hash):
	filepath = join(getwcd(), '__temp.dirhash')
	fobj = open(filepath, 'wb')
	pickle.dump(dir_hash, fobj)
	return filepath
...
def compare_dir_hashes(dh1, dh2):
	'''Changes to apply to dir 2, so that it is synced with dir 1
	'''
	l1 = dh1.items()
	l2 = dh2.items()
	l1.sort(key=lambda arg: sum([ord(c) for c in arg[0][1]]))
	l2.sort(key=lambda arg: sum([ord(c) for c in arg[0][1]]))
	...
	while i1 < length1 and i2 < length2:
		...
		if obj_path1 == obj_path2:
			if obj_type1 == 'f':
				if file_data1 != file_data2:
					yield ('update', obj_path1, obj_path2)
			elif obj_type1 == 'd':
				yield compare_dir_hashes(dh1[key1], dh2[key2])
			else:
				raise NotImplementedError(f'Unsupported object type {obj_type}')
		else:
			yield ('missing', obj_type, obj_path1)

def sync_dir(dir_path, change_list):
	for change in change_list:
		apply_change(dir_path, change)

def apply_change(dir_path, change):
